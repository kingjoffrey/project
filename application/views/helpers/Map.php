...
class Zend_View_Helper_Map extends Zend_View_Helper_Abstract
{
    ...
    public function map($mapId)
    {
        ...
        $miniWidth = $map['mapWidth'] * 10;
        $miniHeight = $map['mapHeight'] * 10;

        $this->view->placeholder('map')->append('<img id="mapImage" src="/img/maps/' . $map['mapId'] . '.png" width="' . $miniWidth . 'px" height="' . $miniHeight . '"/>');
