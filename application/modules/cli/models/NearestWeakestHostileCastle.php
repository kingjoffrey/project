        ...
        foreach ($players->getKeys() as $color) {
            ...
            foreach ($castles->getKeys() as $castleId) {
                ...
                if (!$model_StrengthOfMyEnemy->isStronger()) {
                    ...
                    $this->_castles[$castleId] = $numberOfTurnsNeeded + $model_StrengthOfMyEnemy->getEnemyStrength();
                }
...
//        }
    ...
    private function getCastleId(Cli_Model_Game $game, $playerColor, Cli_Model_Army $army)
    {
        ...
        foreach ($this->_heuristics as $k => $castleId) {
            ...
            $es = new Cli_Model_StrengthOfMyEnemy($army, $game, $castle->getX(), $castle->getY(), $playerColor, 10);
            if (!$es->getMyStrength()) {
                return $castleId;
            } else {
